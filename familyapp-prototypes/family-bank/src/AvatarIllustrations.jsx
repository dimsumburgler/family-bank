...
export function ChildAvatar({ child, isSelected, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`relative group transition-all duration-300 flex-shrink-0 p-3 ${isSelected ? 'scale-105' : 'hover:scale-102'}`}
    >
...
export function AddChildAvatar({ onClick }) {
  return (
    <button 
      onClick={onClick}
      className="flex flex-col items-center gap-2 opacity-50 hover:opacity-100 transition-opacity p-3"
    >
